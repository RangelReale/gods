...
func TestListNew(t *testing.T) {
	...
	if _, ok := list1.Get(0); ok {
		t.Errorf("Got %t expected %t", ok, false)
	}
}
...
func TestListAny(t *testing.T) {
	list := New[string]()
	list.Add("a", "b", "c")
	anyv := list.Any(func(index int, value string) bool {
		return value == "c"
	})
	if anyv != true {
		t.Errorf("Got %v expected %v", anyv, true)
	}
	anyv = list.Any(func(index int, value string) bool {
		return value == "x"
	})
	if anyv != false {
		t.Errorf("Got %v expected %v", anyv, false)
	}
}
...
func benchmarkGet[T constraints.Integer](b *testing.B, list *List[T], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Get(n)
		}
...
func benchmarkAdd[T constraints.Integer](b *testing.B, list *List[T], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Add(T(n))
		}
...
func benchmarkRemove[T constraints.Integer](b *testing.B, list *List[T], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Remove(n)
		}
